...
crossbar.prototype.init = function() {
    ...
    filesName.forEach(function(fileName) {
        ...
        for (var api in apis) {
            ...
            self.init_api(name, api, apiData);
        }

    });
};

crossbar.prototype.init_api = function(name, api, apiData) {
    var self = this;

    self.api[name][api] = function() {
...
crossbar.prototype.build_uri = function(url, data) {
    ...
    if (data.filters) {
        if (Object.keys(data.filters).length === 1) {
            for (let key in data.filters) {
                uri += `?${key}=${data.filters[key]}`;
            }
        }
    }
